...
proved, as the linearity is a very mathematically convenient property for building theories upon. %% couldn't find reference to support this though, is it okay?

One particular direction in which we would like to further explore is experimentally verification.
While we are confident in our proofs, popular reinforcement learning algorithms often subtly deviates from
the universal intelligent agent model by effectively letting the agent look at the future, by playing the same game more than once in a separate training phase.
...
%  Copied from Samuel's mails
%A collection of RL environments where the "step" and "reset" methods return probability distributions rather than individual percepts. (Could fork openai gym or some other existing library, perhaps.)
%A collection of RL agents whose "act" methods return probability distributions rather than individual actions, and who are trained using a "train" method taking tuple (o_prev, action, reward, o_next, done) as input.
