...
\label{firsttechlemmaforgenericity}
    ...
    Suppose $h$ is a history such that $h_0$ is not an initial segment of $h$.
        ...
        &{} r_1\pi^1_*(t) + \cdots + r_n\pi^n_*(t)\\
            ...
            &= \frac{\pi_*(t)}{\pi(a_0|h)}
                (r_1m_1(a_0) + \cdots + r_nm_n(a_0))
                    &\mbox{(Basic Algebra)}\\
            &= \frac{\pi_*(t)}{\pi(a_0|h)}(r_1m_1+\cdots+r_nm_n)(a_0)
                    &\mbox{(Definition \ref{sumofdistros})}\\
    ...
    \begin{align*}
        &{} (r_1\pi^1\oplus\cdots\oplus r_n\pi^n)_*(s)\\
            ...
            &= \pi_*(t)\frac{r_1\pi^1_*(t\frown a)+\cdots+r_n\pi^n(t\frown a)}
                {r_1\pi^1_*(t)+\cdots+r_n\pi^n(t\frown a)}
                    &\mbox{(Definition \ref{maindefn})}\\
            &= \pi_*(t)\frac{r_1\pi^1_*(t\frown a)+\cdots+r_n\pi^n(t\frown a)}
                {\pi_*(t)}
                    &\mbox{(By $*$)}\\
...
\begin{theorem}
    ...
    $\pi$ be any agent.
    For any history $h$ ending in a percept,
    ...
    \begin{enumerate}
        \item For each $i=1,\ldots,n$, $\Upsilon(\pi_{h\mapsto m_i})\geq \Upsilon(\pi)$.
        \item For some $i=1,\ldots,n$, $\Upsilon(\pi_{h\mapsto m_i})>\Upsilon(\pi)$.
    \end{enumerate}
\end{theorem}
