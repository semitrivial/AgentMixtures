...
\begin{definition}
    ...
    and $s\in\mathcal H$, then we define:
    \begin{itemize}
...
\label{maintheorem}
    ...
    For all agents $\vec\pi=(\pi^1,\ldots,\pi^n)$ and positive reals
    $\vec w=(w_1,\ldots,w_n)$ with $w_1+\cdots+w_n=1$, for every
    environment $\mu$,
    if $V^{\pi^1}_\mu,\ldots,V^{\pi^n}_\mu$ all converge, then
    \[
        V^{\vec w\cdot\vec\pi}_\mu
        =
        w_1V^{\pi^1}_\mu + \cdots + w_nV^{\pi^n}_\mu.
    \]
\end{theorem}
...
\begin{proof}
    ...
    =
    w_1V^{\pi^1}_{\mu,n}+\cdots+w_nV^{\pi^n}_{\mu,n})
    $.
    Let $n\in\mathbb N$. Let $S_n$ be the set of all length-$2(n+1)$ percept-action
    sequences.
    By Lemma \ref{basicprobabilitylemma}, it suffices to show that
    $
    (\vec w\cdot\vec\pi,\mu)_*(s)
    =
    w_1(\pi^1,\mu)_*(s)+\cdots+w_n(\pi^n,\mu)_*(s)
    $
    for each
