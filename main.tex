...
\begin{definition}
    ...
    \begin{itemize}
        \item ${P^{\vec\pi}}(h)=(\pi^1_*(h),\ldots,\pi^n_*(h))$.
...
\label{maintheorem}
    ...
    \begin{enumerate}
        ...
        $(\vec w\cdot\vec\pi)_*(h)=\vec w\cdot({P^{\vec\pi}}(h))$.
        \item
        For any $h\in\mathcal H$,
...
we may unambiguously write $\vec w\cdot {P^{\vec\pi}}(h)$
for either $(\vec w\cdot\vec\pi)_*(h)$ or $\vec w\cdot({P^{\vec\pi}}(h))$.
...
\begin{proof}
    ...
    Case 1: $h=\langle\rangle$. Then
    $\pi_*(h)=\vec w\cdot{P^{\vec\pi}}(h)=1\not=0$, so
    vacuously $\pi_*(h)=0$ iff $\vec w\cdot{P^{\vec\pi}}(h)=0$
    (proving condition 1).
    ...
    Case 3: $h=h_0\circ p$ for some $h_0\in (\mathcal P\mathcal A)^*$,
        ...
        $\vec w\cdot{P^{\vec\pi}}(h)=w_1\pi_*(h)+\cdots+w_n\pi_*(h)=\pi_*(h)$.
