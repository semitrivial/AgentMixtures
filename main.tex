...
\begin{abstract}
    Inspired by recent progress in multiagent reinforcement learning,
    in this work we examine the collective intelligent behaviour
    of theoretical universal agents by
    introducing a weighted linear mixture operation on them.
    The operation leads to the
    following result: the expected total reward the mixture agent
    receives in any environment is the corresponding weighted average
    of the expected total rewards the original agents receive in that
    environment.
    Beyond formalizing and
    strengthening a previous informal result of Alexander and Hutter (((Is this appropriate for double-blind???))), this result
    enables
...
Multi-agent reinforcement learning, as with other flavors of reinforcement learning, has been enjoying increased attention in artificial intelligence research.
Both further promises and challenges have been frequented, particularly signified by the
high-profile attempt at replicating the triumph of AI game-playing at more complex realtime video games \cite{nature-starcraft}
than previous well-known AI game-playing achievements based on deep reinforcement learning \cite{nature-ATARI,nature-go-2}.
...
Another close related concept is the collective behavior of intelligent agents.
...
In this section, we will investigate three symmetry properties
which a weighted intelligence measure might satisfy. A priori, these
three properties do not appear to be equivalent, but we will show
that in fact, they are. Throughout this section, we
...
The following auxiliary lemmas will be used in our proof of Lemma \ref{...}
