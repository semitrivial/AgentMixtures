...
\label{stronglywellbehaveddefn}
    ...
    underlying $\Upsilon$ add up to $1$, and any environment that is not strongly
...
\label{mixtureenvdefn}
    ...
    sequence of environments. We define an environment $\vec w\cdot\vec\mu$ by:
    \[
...
\label{tannerysthm}
    ...
    Assume there is a sequence $\{w_i\in (0,\infty)\}_{i=1}^\infty$
...
\label{envmaintheorem}
    ...
    a sequence of environments, and let $\pi$ be any agent. Then:
    \begin{enumerate}
        \item
...
\begin{proof}
    ...
    \end{align*}
    For each $i\in\mathbb N$, $a_i:\mathbb N\to\mathbb R$
    by $a_i(t)=w_iV^\pi_{\mu_i,t}$.
    Since each $\mu_i$ is strongly well-behaved, each $-1\leq V^\pi_{\mu_i,t}\leq 1$.
    It follows that for all $i>0$, for all $t\in\mathbb N$, $|a_i(t)|\leq w_i$.
