...
In fact, multi-agent reinforcement learning had been first introduced as ``collective learning'' \cite{collective-1993} shortly before ever being explicitly called out as multi-agent reinforcement learning \cite{multiagent-1994-most-cited}.
...
One significant recent trend in machine learning (beyond just reinforcement learning)
is Federated Reinforcement Learning \cite{federated-learning-2021},
which is mainly about programs physically running on disparate devices collaborating to form a more powerful artificial intelligence, and conceptually borrows from how humans collaborate.
In this regard, reinforcement learning had a much earlier head start with
...
It is worth noting that "collaborative" reinforcement learning sometimes refers to collaboration between human users and artificial agents, rather than that between artificial agents \cite{collab-other}.

In this work, we draw inspiration from sortition, yet another way of social organization with an ancient root in the
historical Athenian democracy \cite{sortition-ancient} -
actually predating both feudalism and federations.
Sortition itself has attracted recent scientific curiosity and even advocacy into real-world governance \cite{nature-sortition,Sintomer2018FromDT,sortition-2013}.
%to formally examine the crowd intelligence.

More specifically, we formally examine the expected intelligence of a single combined
mixture agent formed from a group of agents using sortition.
Given agents $\vec{\pi}=(\pi_1,\ldots,\pi_n)$ and positive real
number weights
$\vec{w}=(w_1,\ldots,w_n)$ with $w_1+\cdots+w_n=1$, imagine an agent $\sigma$
...
implemented as a new agent who, upon initiation, uses a random number generator
...
Such functions have no ``\_\_init\_\_'' method, no access to a true random number
...
facilitate rigorous mathematical proofs about properties of $\sigma$. To see why this
is non-trivial, consider instead an agent $\rho$ who, upon initiation, examines the
computer's system clock, determine to play as $\pi_1$ if the clock says ``AM'' or as
    ...
    \begin{align*}
        V^\pi_{\mu_\Upsilon}
            ...
            &= \sum_{i=1}^\infty w_i V^\pi_{\mu_i},
                &\mbox{(Definition \ref{infinitedimensionalvectorizationdefn})}
    \end{align*}
    which is $\sum_{\mu\in W}w_\mu V^\pi_\mu$ since $w_\mu=0$ whenever
    $\mu$ is not strongly well-behaved; and this is $\Upsilon(\pi)$
    by Definition \ref{performanceaveragerdefn}.
\end{proof}
...
% This is to be separated into two files during submission
% Reset counters
