...
Elements of $\Delta \mathcal A$ are also known as \emph{policies}.
...
\label{omnibusdefn}
    ...
    \begin{enumerate}
        ...
        $(\pi(\cdot|h))(y)$.
        Intuitively, $\pi$ takes a history $h$ and outputs a policy
        $\pi(\cdot|h)$; for any action $y$,
        $\pi(y|h)$ is the probability that agent $\pi$
...
Intuitively, $\vec w\cdot\vec\pi$ can be thought of as being driven by an entity
who believes that actions are to be chosen by one of the $\vec\pi$, but does not
know which. The entitiy initially assigns each $w_i$ to $\pi_i$ as a prior, and
attempts to guess the probability of each action being chosen by the unknown
agent $\pi_i$, using these priors to do so. As new actions are seen, said priors
...
\begin{definition}
    ...
    if both:
    \begin{enumerate}
        \item $\Upsilon(\pi)\geq \Upsilon(\pi_i)$ for each $i=1,\ldots,n$; and
...
We are optimistic about additional advances in the future
based on our mixture construction. The commutativity of $\vec w$ (Theorem \ref{maintheorem})
is a kind of linearity property.
Such linearity properties are crucial in many areas.
In particular, linearity is a critical assumption in conventional control theory \cite{control-textbook-1,control-textbook-2}.
RL has come full circle: once an offshoot of
optimal control \cite{Sutton2005ReinforcementLA-mentions-optimal-control},
reinforcement learning is now contributing back
thereto, such as in
robotics \cite{RL-robotics,Li2021ReinforcementLF},
...
% resembling for instance the Nyquist stability in control theory \cite{Valena1980NyquistCF}, may become very useful for safe reinforcement learning applications in the physical world \cite{pmlr-v119-stooke20a,Cheng2019EndtoEndSR}.

%While we are confident in our proofs,
%xperimental verification could also be interesting.
%Popular reinforcement learning algorithms often subtly deviates from
%the universal intelligent agent model by playing the same game more than once in a separate training phase.
