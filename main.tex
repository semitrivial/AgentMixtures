...
\label{omnibusdefn}
    ...
    \begin{enumerate}
        ...
        So elements of $(\mathcal P\mathcal A)^*$ of length at least $4$ have the
        ...
        So elements of $(\mathcal P\mathcal A)^* \mathcal P$
        of length at least $5$ have the form
        ...
        $(p_0,a_0,\ldots,p_t,a_t)$ randomly generated as follows:
        \begin{enumerate}
...
\label{pullbackdef}
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item
            If $h=h_0\frown p$ for some $p\in\mathcal P$ then $\pi_*(h)=\pi_*(h_0)$.
            \item
            If $h=h_0\frown a$ for some $a\in\mathcal A$ then $\pi_*(h)=\pi_*(h_0)\pi(a|h_0)$.
        \end{itemize}
        The intended intuition is that $\pi_*(h)$ can be thought of as the
        conditional probability that when $\pi$
        and any environment
        $\mu$ interact, the interaction will begin with $h$, \emph{assuming}
        ...
        \begin{itemize}
            ...
            \item
            If $h=h_0\frown p$ for some $p\in\mathcal P$ then $\mu_*(h)=\mu_*(h_0)\mu(p|h_0)$.
            \item
            If $s=h_0\frown a$ for some $a\in\mathcal A$ then $\mu_*(h)=\mu_*(h_0)$.
        \end{itemize}
        ...
        the interaction will begin with $h$, \emph{assuming} that
        ...
        \begin{itemize}
            ...
            \item
            If $h=h_0\frown p$ for some $p\in\mathcal P$ then
            $(\pi,\mu)_*(h)=(\pi,\mu)_*(h_0)\mu(p|h_0)$.
            \item
            If $h=h_0\frown a$ for some $a\in\mathcal A$ then
            $(\pi,\mu)_*(h)=(\pi,\mu)_*(h_0)\pi(a|h_0)$.
        \end{itemize}
...
\begin{proof}
    ...
    Case 2: $h=h_0\frown a$ for some $a\in\mathcal A$.
        The lemma follows by parts 1 and 3 of Definition \ref{pullbackdef}.

    Case 3: $h=h_0\frown p$ for some $p\in\mathcal P$.
        The lemma follows by parts 2 and 3 of Definition \ref{pullbackdef}.
\end{proof}
...
\begin{proof}
    ...
    show that for every well-behaved $\mu$ and every $n\in\mathbb N$,
    \[
        V^{\pi}_{\mu,n}
        =
        V^{\vec w\cdot \vec\pi}_{\mu,n}.
    \]
    By Lemma \ref{basicprobabilitylemma}, it suffices to show that for every
    well-behaved $\mu$ and every $s\in\mathcal H$,
    $
    (\pi,\mu)_*(s)
    =
    (\vec w\cdot\vec\pi,\mu)_*(s)
    $.
    We will show even more. We will show that for every $s\in\mathcal H$,
    $\pi_*(s)=(\vec w\cdot\vec\pi)_*(s)$.
    ...
    \begin{align*}
        (\vec w\cdot\vec\pi)_*(s)
            &= (\vec w\cdot\vec\pi)_*(t)
            (\vec w\cdot\vec\pi)(a|t)
                &\mbox{(Definition \ref{pullbackdef})}\\
            &= \pi_*(t)(\vec w\cdot\vec\pi)(a|t)
                &\mbox{(Induction)}\\
            &= 0,
    \end{align*}
    ...
    We would like to use Definition \ref{maindefn} to rewrite
    $(\vec w\cdot\vec\pi)(a|t)$ but first we must
    check that the appropriate denominator does not vanish. We compute ($**$):
    \begin{align*}
        \vec w\cdot\vec\pi_*(t)
            &= \frac{w_1\pi_*(t)m_1(a_0)}{\pi(a_0|h)}
                + \cdots +
                \frac{w_n\pi_*(t)m_n(a_0)}{\pi(a_0|h)}
    ...
    \begin{align*}
        (\vec w\cdot\vec\pi)_*(s)
            ...
            &= \pi_*(t)(\vec w\cdot\vec\pi)(a|t)
                    &\mbox{(Induction)}\\
            &= \pi_*(t)\frac{\vec w\cdot\vec\pi_*(t\frown a)}
                {\vec w\cdot\vec\pi_*(t)}
                    &\mbox{(Definition \ref{maindefn})}\\
            &= \vec w\cdot\vec\pi_*(t\frown a)
                    &\mbox{(By $**$)}\\
            &= w_1\frac{\pi_*(t\frown a)m_1(a_0)}{\pi(a_0|h)}
                +\cdots+w_n\frac{\pi_*(t\frown a)m_n(a_0)}{\pi(a_0|h)}
                    &\mbox{(Lemma \ref{secondtechlemmaforgenericity})}\\
            &= \frac{\pi_*(t\frown a)}{\pi(a_0|h)}(w_1m_1(a_0)+\cdots+w_nm_n(a_0))
                    &\mbox{(Basic Algebra)}\\
    ...
    We would like to use Definition \ref{maindefn} to rewrite
    $(\vec w\cdot\vec\pi)(a|h)$ but first we must
    check that the appropriate denominator does not vanish:
    by induction, we have
    ($***$) $\vec w\cdot\vec\pi_*(t)=\pi_*(t)\not=0$.
    Thus
    \begin{align*}
        (\vec w\cdot\vec\pi)_*(t\frown a)
            &= (\vec w\cdot\vec\pi)_*(t)
                (\vec w\cdot\vec\pi)(a|t)
                    &\mbox{(Definition \ref{pullbackdef})}\\
            &= (\vec w\cdot\vec\pi)_*(t)\frac{\vec w\cdot\vec\pi_*(t\frown a)}
                {\vec w\cdot\vec\pi_*(t)}
                    &\mbox{(Definition \ref{maindefn})}\\
            &= \vec w\cdot\vec\pi_*(t\frown a)
                    &\mbox{(By $***$)}\\
            &= w_1\pi_*(t\frown a)+\cdots+w_n\pi_*(t\frown a)
                    &\mbox{(Lemma \ref{firsttechlemmaforgenericity})}\\
            &= \pi_*(t\frown a),
