...
More generally, given agents $\pi^1,\ldots,\pi^n$ and positive reals
...
into $\pi^1$ (with probability $r_1$) or into $\pi^2$ (with probability $r_2$),
...
an agent $w_1\pi^1\oplus\cdots \oplus w_n\pi^n$, called a \emph{mixture agent},
...
rewards of $\pi^1,\ldots,\pi^n$ in that environment.
...
\label{maindefn}
    (Mixture agents)
    Suppose $\pi^1,\ldots,\pi^n$ are agents and $w_1,\ldots,w_n$ are positive
    real numbers with $w_1+\cdots+w_n=1$.
    Define the \emph{mixture agent} $w_1\pi^1\oplus\cdots\oplus w_n\pi^n$
    as follows. For every $s\in(\mathcal P\mathcal A)^*\mathcal P$:
    \begin{itemize}
        \item
        If $w_1\pi^1_*(s)+\cdots+w_n\pi^n_*(s)\not=0$ then
        \[
            (w_1\pi^1\oplus\cdots\oplus w_n\pi^n)(a|s)
            =
            \frac
            {w_1\pi^1_*(s\frown a)+\cdots+w_n\pi^n_*(s\frown a)}
            {w_1\pi^1_*(s)+\cdots+w_n\pi^n_*(s)}.
        \]
        \item
        Otherwise, $(w_1\pi^1\oplus\cdots\oplus w_n\pi^n)(a|s)=1/|\mathcal{A}|$.
    \end{itemize}
\end{definition}

\begin{lemma}
    If $\pi^1,\ldots,\pi^n$ and $w_1,\ldots,w_n$ are as in Definition \ref{maindefn}
    then $w_1\pi^1\oplus\cdots\oplus w_n\pi^n$ really is an agent.
\end{lemma}

\begin{proof}
    We must show
    $\sum_{a\in\mathcal A}(w_1\pi^1\oplus\cdots\oplus w_n\pi^n)(a|s)=1$ for every
    $s\in(\mathcal P\mathcal A)^*\mathcal P$. Fix some such $s$.

    Case 1: $w_1\pi^1_*(s)+\cdots+w_n\pi^n_*(s)=0$. Then
    each $(w_1\pi^1\oplus\cdots\oplus w_n\pi^n)(a|s)=1/|\mathcal A|$ so the
    claim is immediate.

    Case 2: $w_1\pi^1_*(s)+\cdots+w_n\pi^n_*(s)\not=0$. Then
    \begin{align*}
        &{} \sum_{a\in\mathcal A}(w_1\pi^1\oplus\cdots\oplus w_n\pi^n)(a|s)\\
            &= \sum_{a\in\mathcal A}
                \frac
                {w_1\pi^1_*(s\frown a)+\cdots+w_n\pi^n_*(s\frown a)}
                {w_1\pi^1_*(s)+\cdots+w_n\pi^n_*(s)}
            ...
            &= \frac
                ...
                {w_1\pi^1_*(s)+\dots+w_n\pi^n_*(s)}=1.
...
\begin{proof}
    ...
    Thus by Corollary \ref{averageperformancelemma},
    ...
    \begin{align*}
        \Upsilon(\pi)
            ...
            &= r_1\Upsilon(\pi^{h\mapsto m_1})+\cdots+r_n\Upsilon(\pi^{h\mapsto m_n})
                &\mbox{(Corollary \ref{averageperformancelemma})}\\
